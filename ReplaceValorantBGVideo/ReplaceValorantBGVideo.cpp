...
void ReplaceValorantBGVideo::replaceVideo(QString File)
{
	...
	// 获取文件信息
	QString Video_FileName = QFileInfo(File).fileName();
	QString Temp_Game_File = "./temp/" + GAMEFILE_NAME;

	// 输出调试信息
	qDebug() << "游戏视频文件名：" << GAMEFILE_NAME << "\n"
		<< "游戏视频路径：" << GAMEPATH << "\n"
		<< "替换视频文件名：" << Video_FileName << "\n"
		<< "替换视频路径：" << QFileInfo(VideoDir).absolutePath() << "\n";
	...
	// 复制替换视频到临时目录
	if (!QFile::copy(File, Temp_Game_File)) {
		qDebug() << "复制视频到临时目录失败";
	...
	qDebug() << "操作完成：" << "\n"
		<< "备份文件位于：" << backupFilePath << "\n"
		<< "新视频已替换至：" << GAMEPATH << "\n";
}

//游戏进行中时，如果意外关闭程序 也能恢复原文件（不能？！）
